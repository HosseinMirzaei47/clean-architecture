...
import comtest.dandeliontest.todotest.task.ToDoFilterQueryBuilderConstants.QUERY_OPERATOR_TYPE_BIGGER_THAN
import comtest.dandeliontest.todotest.task.ToDoFilterQueryBuilderConstants.QUERY_OPERATOR_TYPE_EQUAL
import comtest.dandeliontest.todotest.task.ToDoFilterQueryBuilderConstants.QUERY_OPERATOR_TYPE_NOT_EQUAL
import comtest.dandeliontest.todotest.task.ToDoFilterQueryBuilderConstants.QUERY_OPERATOR_TYPE_SMALLER_THAN
import comtest.dandeliontest.todotest.task.ToDoFilterQueryBuilderConstants.SELECT_OPERATOR_AND

// TODO: 4/16/21 This Class (And All Data Classes) Should Move To Best Destination. Please Do It After Apply New Architecture.
class ToDoFilterQueryBuilder(private val defaultToDoFilterSort: ToDoFilterSortMethod) {

    private val selectItems = "taskId,title,description,dueDate,dueTime,isDone,priority"
    ...
    * */
    fun addSortQuery(vararg toDoFilterSortMethods: ToDoFilterSortMethod) {
        toDoFilterSortMethods.forEachIndexed { index, sortMethod ->
            ...
            if (sortMethod.putNullsEnd) {
                orderByQuery += " CASE WHEN ${sortMethod.nameOfColumn} IS NULL THEN 1 ELSE 0 END, "
            }
            orderByQuery += " ${
